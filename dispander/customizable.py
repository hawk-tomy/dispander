...
def get_user(
    user: User | Member, icon: Optional_[str], name: Optional_[str]
) -> EllipsisType | UserMock:
...
def cache(
    func: Callable[Concatenate[Id, P], Awaitable[T] | T]
...
class Customizer:
    ...
    def set_message(
        self, customizer: MessageCustomizerFunc, *, enable_cache: bool = True
    ) -> Self:
        if enable_cache:
    ...
    def set_guild(
        self, customizer: GuildCustomizerFunc, *, enable_cache: bool = True
    ) -> Self:
        if enable_cache:
    ...
    def channel(self) -> ChannelCustomizerFunc:
        try:
            ...
            def inner(channel: MessageableChannel) -> ChannelCustomized:
    ...
    def set_channel(
        self, customizer: ChannelCustomizerFunc, *, enable_cache: bool = True
    ) -> Self:
        if enable_cache:
    ...
    def set_attachment(
        self, customizer: AttachmentCustomizerFunc, *, enable_cache: bool = True
    ) -> Self:
        if enable_cache:
    ...
    async def extract_message(self, message: Message) -> list[Message]:
        ...
        for message in messages:
            ...
            assert not isinstance(
                message.channel, (DMChannel, GroupChannel, PartialMessageable)
            )
            ...
            for attachment in message.attachments:
                attachments.append(
                    await maybe_coroutine(self.customizer.attachment, attachment)
                )
